  ...
  return (
    <nav className="bg-urban-green text-white py-5 px-6 md:px-12 flex items-center justify-between sticky top-0 z-50 shadow-md">
      ...
      <div className="hidden md:flex items-center gap-8 text-[13px] font-medium tracking-tight">
        ...
        {!user ? (
          <button onClick={onOpenAuth} className="flex items-center gap-2 bg-white text-urban-green px-6 py-2.5 rounded-xl hover:bg-gray-50 transition shadow-sm font-semibold uppercase tracking-widest text-[11px]">
            Login
          </button>
        ) : (
          <div className="flex items-center gap-5">
             <button onClick={onLogout} className="opacity-60 hover:opacity-100 transition font-semibold uppercase tracking-widest text-[10px]">Logout</button>
             <div className="w-10 h-10 rounded-xl overflow-hidden border border-white/20 flex items-center justify-center bg-white/10">
               {user.avatar_url ? <img src={user.avatar_url} className="w-full h-full object-cover" alt="Profile" /> : <span className="font-semibold text-xs">{user.full_name?.charAt(0)}</span>}
             </div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-[60] md:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-4/5 max-w-sm bg-white text-gray-800 p-8 flex flex-col shadow-2xl">
            <div className="flex justify-between items-center mb-10">
              <span className="font-semibold text-xl text-urban-green tracking-tight">UrbanNest</span>
              <button onClick={() => setIsMenuOpen(false)} className="text-gray-400"><i className="fas fa-times text-xl"></i></button>
            </div>
            
            <div className="flex flex-col gap-4">
              <Link to="/" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-4 p-5 rounded-2xl bg-gray-50 text-gray-900 font-medium text-sm">
                <i className="fas fa-home text-urban-green"></i> Home
              </Link>
              
              {user ? (
                <>
                  <Link to="/chat" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-4 p-5 rounded-2xl bg-gray-50 text-gray-900 font-medium text-sm">
                    <i className="fas fa-comment-dots text-urban-green"></i> Chat
                  </Link>
                  <Link to="/profile" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-4 p-5 rounded-2xl bg-gray-50 text-gray-900 font-medium text-sm">
                    <i className="fas fa-user text-urban-green"></i> Profile
                  </Link>
                  {user.role === 'owner' && (
                    <Link to="/dashboard" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-4 p-5 rounded-2xl bg-gray-50 text-gray-900 font-medium text-sm">
                      <i className="fas fa-table-columns text-urban-green"></i> My Ads
                    </Link>
                  )}
                  <button onClick={() => { onLogout(); setIsMenuOpen(false); }} className="flex items-center gap-4 p-5 rounded-2xl text-red-500 bg-red-50 font-medium text-sm text-left w-full mt-4">
                ...
                >
                  ...
                  <span className="font-semibold uppercase tracking-widest text-xs">Login / Sign Up</span>
                </button>
              )}
            </div>
          </div>
