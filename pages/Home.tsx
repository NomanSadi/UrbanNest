...
const Home: React.FC = () => {
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="bg-urban-green pt-16 pb-28 px-6 md:px-12 relative overflow-hidden text-center">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="mb-16 space-y-4">
             <h1 className="text-4xl md:text-6xl font-semibold text-white leading-tight tracking-tight">
                Find your <br/>
                <span className="text-white/40 italic font-medium">urban nest.</span>
             </h1>
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-10">
            {['Gulshan', 'Banani', 'Dhanmondi', 'Uttara', 'Mirpur'].map(loc => (
              <button 
                ...
                className="bg-white/5 hover:bg-white/15 text-white/50 hover:text-white text-[9px] font-semibold uppercase tracking-widest px-4 py-2 rounded-lg border border-white/5 transition-all"
              >
                {loc}
              </button>
            ))}
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between md:justify-around px-6 gap-8 min-w-max">
          {categories.map((cat, idx) => (
            ...
            >
              ...
              <span className="hidden sm:inline font-semibold uppercase tracking-widest text-[10px]">{cat.name}</span>
            </button>
          ))}
        </div>
      </div>
        ...
        <section className="mb-20">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <div className="space-y-2">
              <span className="text-urban-green text-[10px] font-semibold uppercase tracking-[0.3em] block ml-1">
                {activeCategory !== 'All' ? activeCategory : 'Featured Collection'}
              </span>
              <h2 className="text-2xl md:text-3xl font-semibold text-gray-900 tracking-tight leading-none">
                {activeCategory === 'Budget' ? 'Best Values' : 'Premium Rentals'}
              </h2>
            </div>
            {activeCategory !== 'All' && (
              <button onClick={() => setActiveCategory('All')} className="text-[10px] font-semibold text-gray-400 hover:text-urban-green uppercase tracking-widest bg-gray-50 px-6 py-3 rounded-xl transition-all self-start md:self-auto border border-gray-100">
                Reset Filters
              </button>
            )}
          </div>
          ...
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {[1, 2, 3].map(i => <div key={i} className="h-[400px] bg-gray-50 rounded-3xl animate-pulse"></div>)}
            </div>
            ...
            <div className="text-center py-32 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
              <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-sm">
                 <i className="fas fa-house-chimney-crack text-2xl text-gray-200"></i>
              </div>
              <h3 className="text-xl font-semibold text-gray-900 mb-2">No matching homes</h3>
              <p className="text-gray-400 max-w-sm mx-auto mb-8 text-sm font-medium">Try adjusting your filters or search in a different area.</p>
              <button onClick={() => { setSearch(''); setActiveCategory('All'); }} className="bg-urban-green text-white px-8 py-3.5 rounded-xl font-semibold text-[10px] uppercase tracking-widest shadow-lg shadow-urban-green/20">Clear All Filters</button>
            </div>
          )}
        </section>
      </main>
